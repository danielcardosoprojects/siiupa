...
<div id="box_grande">
    ...
    $sqlConsulta_Acionamento = "SELECT ac.*, f.nome, acs.acionamento, acs.id as acionamentosId, f.fk_cargo as cargoId, cargo.funcao_upa FROM db_rh.tb_acionamento as ac inner join db_rh.tb_funcionario as f on (ac.fk_funcionario = f.id) inner join db_rh.tb_cargo as cargo on (f.fk_cargo = cargo.id) inner join db_rh.tb_acionamentos as acs on(ac.fk_acionamentos = acs.id) where ac.id = '$id_acionamento' ORDER BY ac.id DESC";
    ...
    ?>
        ...
        <div class="box_acionamentos" id="box_<?php echo $resultado_acionamento->id; ?>" data-id="<?php echo $resultado_acionamento->id; ?>" name='<?php echo $resultado_acionamento->nome ?>'>
            ...
            <div class="nome">
                <h5>👤 <a href="#"><?php echo utf8_encode($resultado_acionamento->nome); ?></a><span class="ui-icon ui-icon-copy copiarAcionamento" data-id="<?php echo $resultado_acionamento->id; ?>" data-text=""></span></h5>
            </div>
