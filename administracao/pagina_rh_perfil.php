...
<div class="alert alert-success" role="alert">
    ...
    $sql = "SELECT  DATE_FORMAT(f.admissao,'%d\/%m\/%Y') as admissaobr, DATE_FORMAT(f.data_nasc,'%d\/%m\/%Y') as data_nascbr, f.*, c.titulo AS cargo, c.descricao AS cargo_desc, s.setor FROM tb_funcionario AS f INNER JOIN tb_cargo AS C ON f.fk_cargo = c.id INNER JOIN tb_setor AS s ON f.fk_setor = s.id $where $orderby";
...
<div id='perfil'>
    ...
    <table id="detalhes_cargo" class="table table-hover table-bordered">
        ...
        <tbody>
            ...
            <td><?php echo diveditavel($perfil->id, 'cpf', $perfil->cpf); ?></td><!-- VALOR CPF -->
...
<table id="dados_folhas" class="table  table-bordered  border-dark table-sm table-hover table-striped Tabela_folha" style="font-size:12px">
    ...
    while ($arquivo = $diretorio->read()) {
        ...
        echo "<tr><td><a target='_blank' href='" . $path . $arquivo . "'>" . $arquivo . "</a></td><td><a href='#' class='apagaArquivo' data-arquivo=" . $path . $arquivo . ">$linkLixeira</a></td></tr>";
    }
    echo "</tbody></table>";
    $diretorio->close();
